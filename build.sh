...
[ -d build ] || mkdir build
cd build
cmake .. -DCMAKE_INSTALL_PREFIX=${INSTALL_DIR} -DCMAKE_CXX_COMPILER=hipcc -DUSE_HIP=ON  -DCMAKE_CXX_FLAGS="--offload-arch=gfx90a -O3" -DCMAKE_C_COMPILER=hipcc -DCMAKE_BUILD_TYPE=Release
make -j 64  VERBOSE=1
